<div class="flex h-auto w-full flex-1 flex-col gap-2 rounded-xl">

    <div class="flex justify-between">
        <div>
            <flux:heading size="xl">Industries List</flux:heading>
            <flux:text size="lg">List industri yang ada di Indonesia.</flux:text>
        </div>
        
        <flux:modal.trigger name="add-industry">
            <flux:button class="self-end" wire:click="resetInputFields()">Tambah</flux:button>
        </flux:modal.trigger>
    </div>

    <flux:modal name="add-industry" class="w-144">
        <div class="space-y-4">
            <div>
                <flux:heading size="lg">Tambah Industri</flux:heading>
                <flux:text class="mt-2">Tambah data penting perusahaan.</flux:text>
            </div>
            <div class="grid grid-cols-2 gap-3">
                <flux:input label="Nama" placeholder="Nama Industri" wire:model="nama" required />
                <flux:input label="Email" type="email" placeholder="user@example.com" wire:model="email" required />
            </div>
            <div class="grid grid-cols-2 gap-3">
                <flux:input label="Website" placeholder="https://example.com/" wire:model="website" />
                <flux:input label="Kontak" mask="9999999999999" placeholder="0812-3456-7890" wire:model="kontak" />
            </div>
            <flux:textarea label="Bidang Usaha" resize="none" rows="2" wire:model="bidang_usaha" />
            <flux:textarea label="Alamat" resize="none" rows="3" wire:model="alamat" />
            <flux:button variant="primary" class="w-full" wire:click.prevent="store()">Tambah</flux:button>
        </div>
    </flux:modal>

    <flux:modal name="edit-industry" class="w-144">
        <div class="space-y-4">
            <div>
                <flux:heading size="lg">Edit Industri</flux:heading>
                <flux:text class="mt-2">Edit data penting perusahaan.</flux:text>
            </div>
            <flux:input type="hidden" wire:model="industry_id"/>
            <div class="grid grid-cols-2 gap-3">
                <flux:input label="Nama" placeholder="Nama Industri" wire:model="nama" required />
                <flux:input label="Email" type="email" placeholder="user@example.com" wire:model="email" required />
            </div>
            <div class="grid grid-cols-2 gap-3">
                <flux:input label="Website" placeholder="https://example.com/" wire:model="website" />
                <flux:input label="Kontak" mask="9999999999999" placeholder="0812-3456-7890" wire:model="kontak" />
            </div>
            <flux:textarea label="Bidang Usaha" resize="none" rows="2" wire:model="bidang_usaha" />
            <flux:textarea label="Alamat" resize="none" rows="3" wire:model="alamat" />
            <flux:button variant="primary" class="w-full" wire:click.prevent="update()">Simpan</flux:button>
        </div>
    </flux:modal>

    <div class="relative h-full flex-1 overflow-hidden rounded-xl border border-neutral-200 dark:border-neutral-700">
        <table class="w-full dark:text-gray-100">
            <thead class="text-gray-100 uppercase">
                <tr class="dark:bg-[#181818]">
                    <th class="px-2 py-2">Nama</th>
                    <th class="px-2 py-2">Email</th>
                    <th class="px-2 py-2">Kontak</th>
                    <th class="px-2 py-2">Details</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($industries as $industry)
                <tr class="border-t dark:border-[#555555] text-sm">
                    <td class="py-3 px-6">{{ $industry->nama }}</td>
                    <td class="py-3 px-6">{{ $industry->email }}</td>
                    <td class="py-3 px-6 text-center">{{ $industry->kontak }}</td>
                    <td class="py-2 px-3">
                        <div class="flex align-center justify-center gap-2">
                            <flux:modal.trigger name="edit-industry">
                                <flux:button icon="ellipsis-horizontal" size="sm" wire:click="edit({{ $industry->id }})" />
                            </flux:modal.trigger>
    
                            <flux:modal.trigger name="delete-industry">
                                <flux:button icon="trash" variant="danger" size="sm" wire:click="delete({{ $industry->id }})" />
                            </flux:modal.trigger>
                        </div>
                    </td>
                </tr>
                @endforeach
            </tbody>
        </table>
    </div>
    {{ $industries->links() }}
</div>
